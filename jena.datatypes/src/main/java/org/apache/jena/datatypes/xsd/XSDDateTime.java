...
public class XSDDateTime extends AbstractDateTime {
    ...
    private static int[] convertCalendar(Calendar date) {
        ...
        data[CY] = cal.get(Calendar.YEAR);
        data[M] = cal.get(Calendar.MONTH) + 1;
        data[D] = cal.get(Calendar.DAY_OF_MONTH);
        data[h] = cal.get(Calendar.HOUR_OF_DAY);
        data[m] = cal.get(Calendar.MINUTE);
        data[s] = cal.get(Calendar.SECOND);
        data[utc] = 'Z';
