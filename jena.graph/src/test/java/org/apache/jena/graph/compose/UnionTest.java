...
public class UnionTest {
	...
	public UnionTest() {
		graphProducer = new AbstractGraphProducer<Union>() {
			...
			protected Union createNewGraph() {
				Graph[] g = { GraphHelper.memGraph(), GraphHelper.memGraph() };
	...
	public void testUnion() {
		...
		GraphHelper.assertContains("Union", "x R y", u);
		GraphHelper.assertContains("Union", "p R q", u);
		GraphHelper.assertContains("Union", "r Foo s", u);
		if (u.size() != 3)
			fail("oops: size of union is not 3");
		u.add(GraphHelper.triple("cats eat cheese"));
		GraphHelper.assertContains("Union", "cats eat cheese", u);
		if (GraphHelper.contains(g1, "cats eat cheese") == false
				&& GraphHelper.contains(g2, "cats eat cheese") == false)
