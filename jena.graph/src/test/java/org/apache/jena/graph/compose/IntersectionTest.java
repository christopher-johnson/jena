...
public class IntersectionTest {
	...
	public void testIntersection() {
		...
		GraphHelper.assertContains("Intersection", "x R y", i);
		GraphHelper.assertOmits("Intersection", i, "p R q");
		GraphHelper.assertOmits("Intersection", i, "r Foo s");
		if (i.size() != 1)
			fail("oops: size of intersection is not 1");
		i.add(GraphHelper.triple("cats eat cheese"));
		GraphHelper.assertContains("Intersection.L", "cats eat cheese", g1);
		GraphHelper.assertContains("Intersection.R", "cats eat cheese", g2);
	}

	@ContractTest
	public void testDeleteDoesNotUpdateR() {
		Graph L = GraphHelper.graphWith("a pings b; b pings c; c pings a");
		Graph R = GraphHelper.graphWith("c pings a; b pings c; x captures y");
		Graph join = new Intersection(L, R);
		GraphUtil.deleteFrom(L, R);
		GraphHelper.assertIsomorphic("R should not change",
				GraphHelper.graphWith("c pings a; b pings c; x captures y"), R);
		GraphHelper.assertIsomorphic(GraphHelper.graphWith("a pings b"), L);
	}
}
