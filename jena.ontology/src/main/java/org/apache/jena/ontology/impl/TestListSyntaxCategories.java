    ...
    extends TestCase
{
        ...
        extends TestCase
    {
        ...
        public void runTest() {
            ...
            TestCase.assertEquals( "Ontology exception" + (m_exExpected ? " was " : " was not ") + "expected", m_exExpected, exOccurred );
            ...
            if (!exOccurred) {
                ...
                while (i.hasNext()) {
                    Resource res = i.next();
                    TestCase.assertTrue( "Should not fail node test on " + res, test( res ));
                ...
                TestCase.assertEquals( getName() + ": wrong number of results returned", m_count, actual.size() );
                if (expected != null) {
                    TestCase.assertTrue( "Did not find all expected resources in iterator", expected.isEmpty() );
                    TestCase.assertEquals( "Found extraneous results, not in expected list", 0, extraneous );
                }
            }
        }
