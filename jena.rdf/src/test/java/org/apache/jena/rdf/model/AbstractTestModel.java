...
public abstract class AbstractTestModel extends ModelTestBase
    ...
    */
    public void testGetProperty()
        ...
        TestCase.assertTrue( ob.equals( resource( model, "a" ) ) || ob.equals( resource( model, "b" ) ) );
    ...
    public void testGetPropertyWithLanguage()
        ...
        TestCase.assertTrue( "d".equals( l.getString() ) || "e".equals( l.getString() ) );
        }
        ...
        try {
            ...
            TestCase.fail("Must thrown PropertyNotFoundException.");
        } catch (PropertyNotFoundException e) {}
        final Literal l = x.getRequiredProperty( property(model, "R") ).getLiteral();
        TestCase.assertTrue( "d".equals( l.getString() ) || "e".equals( l.getString() ) );
        TestCase.assertTrue("de".equals( l.getLanguage() ) || "fr".equals( l.getLanguage() ) );
        }
        }

    public void testToStatement()
        {
        Triple t = GraphTestBase.triple( "a P b" );
        Statement s = model.asStatement( t );
        assertEquals( GraphTestBase.node( "a" ), s.getSubject().asNode() );
        assertEquals( GraphTestBase.node( "P" ), s.getPredicate().asNode() );
        assertEquals( GraphTestBase.node( "b" ), s.getObject().asNode() );
        }
    ...
    public void testLiteralNodeAsResourceFails()
        ...
        try 
            ...
            TestCase.fail( "should fail to convert literal to Resource" );
            }
	    ...
	    ModelCom mc = (ModelCom) ModelFactory.createDefaultModel();
            for ( String[] aCase : cases )
            {
                for ( int j = 0; j < 3; j += 1 )
                {
                    ...
                    Triple remove = GraphTestBase.triple( aCase[1] );
